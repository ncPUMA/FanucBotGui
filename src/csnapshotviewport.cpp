...
{
    if (d_ptr->scaleSpinbox)
    {
        d_ptr->scaleSpinbox->blockSignals(true);
        d_ptr->scaleSpinbox->setValue(scale);
        d_ptr->scaleSpinbox->blockSignals(false);
    }
...
void CSnapshotViewport::wheelEvent(QWheelEvent *event)
{
    ...
    {
        d_ptr->scaleSpinbox->blockSignals(true);
        d_ptr->scaleSpinbox->setValue(d_ptr->view->Scale());
        d_ptr->scaleSpinbox->blockSignals(false);
    }
}
